...
async def get_stream_from_pytubefix(video_id: str):
    """Primary method: Use pytubefix to get stream"""
    try:
        print(f"  ðŸ”§ Trying pytubefix")
        ...
        if not audio_stream:
            print(f"  âš ï¸  No audio stream found")
            return None
        ...
        print(f"  âœ… Success with pytubefix")
        return result
        
    except Exception as e:
        print(f"  âš ï¸  pytubefix failed: {str(e)[:80]}")
        return None
...
async def get_stream_from_invidious(video_id: str):
    ...
    for instance in INVIDIOUS_INSTANCES:
        try:
            ...
            print(f"  âš ï¸  Invidious {instance}: {str(e)[:60]}")
            continue
...
async def get_stream_from_piped(video_id: str):
    ...
    for instance in PIPED_INSTANCES:
        try:
            ...
            print(f"  âš ï¸  Piped {instance}: {str(e)[:60]}")
            continue
...
async def get_stream_url(video_id: str):
    try:
        ...
        # Try methods in order: pytubefix â†’ Invidious â†’ Piped
        data = None
        
        # Try pytubefix first (most reliable for YouTube)
        data = await get_stream_from_pytubefix(video_id)
        
        # Try Invidious if pytubefix failed
        if not data:
            print(f"âš ï¸  pytubefix failed, trying Invidious...")
        ...
        if not data:
            ...
            raise HTTPException(
                status_code=503, 
                detail="Unable to fetch stream. All methods are temporarily unavailable. Please try again in a moment."
            )
        
        # Cache for 30 minutes (YouTube URLs typically expire after 6 hours)
